...
def main(seed=2025, n_steps=300, q_total=10000, y_scale="lots"):
    ...
    if y_scale=="lots":
        unit = q_total/10.0
        sched_y   = sched/unit
        boundary_y= np.maximum(0, sched - band)/unit
        xs_y, ys_y= xs, ys/unit
        ylab="Cumulative inventory (×{} of total)".format(10)
    elif y_scale=="kshares":
        unit=1000.0; sched_y=sched/unit; boundary_y=np.maximum(0, sched-band)/unit; xs_y, ys_y=xs, ys/unit; ylab="Cumulative inventory (k shares)"
    else:
        unit=1.0; sched_y=sched; boundary_y=np.maximum(0, sched-band); xs_y, ys_y=xs, ys; ylab="Cumulative inventory"

    os.makedirs("results", exist_ok=True)
    fig = plt.figure()
    plt.plot(t, sched_y, linestyle="--", label="Almgren–Chriss target")
    plt.plot(xs_y, ys_y, drawstyle="steps-post", label="Optimal execution (step)")
    if len(trig_idx)>0:
        plt.scatter(t[trig_idx], boundary_y[trig_idx], s=28, color="red", label="Trigger boundary")
    plt.xlabel("Time (s)"); plt.ylabel(ylab); plt.legend()
    fig.savefig("results/ac_path_boundary.png", dpi=160, bbox_inches="tight")
